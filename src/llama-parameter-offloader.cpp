...
parameter_offloader::parameter_offloader(llama_model  * model)
    : model(model), model_i(parameter_offloader_get_model_i(model))
{
...
uint64_t parameter_offloader::hash_dense_graph(ggml_backend_sched_t sched, const ggml_cgraph * graph, std::vector<ggml_tensor *> & graph_nodes) const
{
    (void)sched;

    graph_nodes.clear();

    // 64-bit non-cryptographic fingerprint; assuming uniform outputs, collision probability among n cached graphs is ~n(n-1)/(2*2^64), about 2.7e-14 at n=1000; collisions are not currently verified structurally.
    uint64_t hash = 14695981039346656037ULL;

    // Mix one 64-bit value into the dense-read signature using a SplitMix64-style avalanche.
    ...
    auto hash_node_weights = [&](ggml_tensor * node) {
        ...
        size_t graph_nodes_tensor_count = 0;
        ...
        for (int k = 0; k < GGML_MAX_SRC; ++k)
        {
            ...
            if (w_gpu->buffer != arena)
                continue;

            const char * data = (char *) w_gpu->data;

            if (!data || data < arena_base || data >= arena_base + arena_dense_size)
                continue;

            bool duplicate = false;

            //TODO: Can we move duplicate detection out of hash_dense_graph and into analyze_dense_graph?
            for (size_t i = 0; i < graph_nodes_tensor_count; ++i)
            {
                if (graph_nodes_tensors[i] == w_gpu)
                {
                    duplicate = true;
                    break;
                }
            ...
            graph_nodes_tensors[graph_nodes_tensor_count++] = w_gpu;
        }

        if (graph_nodes_tensor_count == 0)
            return false;

        graph_nodes.push_back(node);

        hash_value(graph_nodes_tensor_count);
    ...
    for (int si = 0; si < sched->n_splits; ++si)
    {
        ...
        for (int j = sp.i_start; j < sp.i_end; ++j)
            found_managed_read |= hash_node_weights(graph->nodes[j]);
    }

    // Fall back to the full graph if no managed dense reads were found in matching splits.
    if (!found_managed_read)
#endif /* if PO_GRAPH_FILTER_BY_BACKEND > 0 */
    {
        for (int i = 0; i < graph->n_nodes; ++i)
            hash_node_weights(graph->nodes[i]);
    }

    hash_value(graph_node_count);

    return hash;
}

void parameter_offloader::analyze_dense_graph(dense_graph_analysis & analysis)
{
    std::unordered_set<ggml_tensor *> seen;
    seen.reserve(gpu2cpu.size());

    const size_t alignment = arena_alignment ? arena_alignment : 1;
    size_t dense_tensor_bytes = 0;

    // Record the managed dense weights read by each node already identified by the graph hash pass.
    for (ggml_tensor * node : analysis.graph_nodes)
    {
        ...
        analysis.graph_nodes_tensors.push_back(std::move(graph_nodes_tensors));
    }
...
void parameter_offloader::streaming_fit_calculate_bounds(dense_graph_analysis & analysis)
{
    ...
    for (int node_idx = 0; node_idx < node_total; ++node_idx)
    {
        ...
        //build a node_group that looks incorporates a number of nodes containing atleast one unique tensor (reading ahead), the number being target_nodes
        auto new_node_group = [&](int target_nodes)
        {
                ...
                j == node_total - 1 ? j = 0 : ++j)
            {
                ...
                {
                    if (std::none_of(tensor_x_node_idx.begin(), tensor_x_node_idx.end(), [&](const auto & entry) { return entry.first == tensor; })
    ...
    auto calculate_bound = [&](std::vector<node_group> & groups)
    {
        ...
        for (const auto & group : groups)
            bound = std::max(bound, group.bytes);

        return bound;
    };

    size_t streaming_tensor_count = 0;
...
void parameter_offloader::build_graph_runtime_metadata(dense_graph_analysis & analysis, const offloader_schedule & schedule, dense_graph_cache_entry & cache_entry)
{
    ...
    cache_entry.release_node_ordinal_by_tensor.assign(tensor_count, -1);
    cache_entry.next_required_tensor_idx_by_node_ordinal.assign(analysis.graph_nodes.size(), -2);
    ...
    int previous_read_node_ordinal = -1;
    ...
    for (size_t read_pos = 0; read_pos < analysis.graph_nodes_tensors.size(); ++read_pos)
    {
        ...
        for (ggml_tensor * w_gpu : analysis.graph_nodes_tensors[read_pos])
        {
            ...
            cache_entry.release_node_ordinal_by_tensor[(size_t)idx] = (int)read_pos;
        ...
        cache_entry.next_required_tensor_idx_by_node_ordinal[read_pos] = -1;
        ...
        if (previous_read_node && node_first_read_idx >= 0)
        {
            ...
            cache_entry.next_required_tensor_idx_by_node_ordinal[(size_t)previous_read_node_ordinal] = node_first_read_idx;
        }

        previous_read_node = node;
        previous_read_node_ordinal = (int)read_pos;
    }

    if (previous_read_node && first_read_idx >= 0)
    {
        analysis.next_required_tensor_idx[previous_read_node] = first_read_idx;
        cache_entry.next_required_tensor_idx_by_node_ordinal[(size_t)previous_read_node_ordinal] = first_read_idx;
    }

    int release_position = -1;
    ggml_tensor * release_node = nullptr;
    int release_node_ordinal = -1;
    ...
    for (size_t i = 0; i < tensor_count; ++i)
    {
        if (read_last_position[i] > release_position)
        {
            ...
            release_node_ordinal = cache_entry.release_node_ordinal_by_tensor[i];
        }

        analysis.release_node_by_tensor[i] = release_node;
        cache_entry.release_node_ordinal_by_tensor[i] = release_node_ordinal;
    }
}
...
size_t parameter_offloader::generate_streaming_fit(offloader_schedule & schedule, const dense_graph_analysis & analysis)
{
    ...
    for (ggml_tensor * tensor : schedule.gpu_tensors_in_order)
    {
        if (tensor_offsets.count(tensor) == 0)
            LLAMA_LOG_ERROR("%s: missing tensor offset %d\n", __func__, schedule.gpu2index.at(tensor));
    ...
    while (!offset_x_tensor_ascending.empty())
    {
        ...
        {
            //sort by node-pair size, largest is first
            ...
            //loop over each duplicate, skipping the first
    ...
    //regenerate the tensor maps for the compaction pass
    for (ggml_tensor * tensor : schedule.gpu_tensors_in_order)
        tensor_x_iterator[tensor] = offset_x_tensor_ascending.insert({ tensor_offsets.at(tensor).first, tensor });
    
    //Now try compacting them. Loop over addresses starting at zero, look for empty space that we can nudge down into
    for (size_t addr = 0; addr < fit_size; addr += alignment)
    {
        std::multimap<size_t, ggml_tensor *>::iterator first = offset_x_tensor_ascending.begin();
        if (first->first == addr)
        {
            //Drop lowest unique address from the search, it wont be moved later. Since every address at the point is garaunteed unique, only need to check the first
            tensor_x_iterator.erase(first->second);
            offset_x_tensor_ascending.erase(first);
            if (offset_x_tensor_ascending.empty())
                break;
            continue;
        }

        //Found a free address, now for each cycle see if the first tensor ahead of addr can be moved down
        for (const std::vector<ggml_tensor *> & tensors : cycles_x_tensors_in_order)
        {
            ggml_tensor * candidate_tensor = NULL;
            size_t last_end = 0;     //there must be space after the previous tensor to the candidate
            for (ggml_tensor * tensor : tensors)
            {
                size_t offset = tensor_offsets[tensor].first;
                if (offset > addr)
                {
                    candidate_tensor = tensor;
                    break;
                }
                last_end = offset + get_gpu_aligned_size(tensor, alignment);
            }

            if (candidate_tensor && addr >= last_end)
            {
                //now check that no COPY HALT conflict will be created, otherwise reject and keep searching
                size_t candidate_size = get_gpu_aligned_size(candidate_tensor, alignment);

                for (int node_idx : tensor_x_node_pairs[candidate_tensor])
                    for (ggml_tensor * tensor : analysis.node_pairs[node_idx].tensors)
                        if (tensor != candidate_tensor)
                        {
                            size_t tensor_start = tensor_offsets.at(tensor).first;
                            size_t tensor_end = tensor_start + get_gpu_aligned_size(tensor, alignment);

                            if (ranges_overlap(addr, addr + candidate_size, tensor_start, tensor_end))
                                goto nudge_candidate_conflict;
                        }

                //Nudge down is successful, move the offset and remove it from the search
                tensor_offsets.at(candidate_tensor).first = addr;
                offset_x_tensor_ascending.erase(tensor_x_iterator.at(candidate_tensor));
                tensor_x_iterator.erase(candidate_tensor);
                break;

            nudge_candidate_conflict:
                ;
            }
        }
    ...
    for (const std::vector<ggml_tensor *> & tensors : cycles_x_tensors_in_order)
    {
        ...
        fit_size = std::max(fit_size, tensor_offsets[tensor].first + get_gpu_aligned_size(tensor, alignment));
    }

    schedule.start_offset.resize(schedule.gpu_tensors_in_order.size());
    schedule.end_offset.resize(schedule.gpu_tensors_in_order.size());
    ...
    if (fit_size > streaming_fit_upper_bound)
    {
        ...
        for (size_t i = 0; i < schedule.gpu_tensors_in_order.size(); ++i)
        {
            auto it = tensor_offsets.find(schedule.gpu_tensors_in_order[i]);
            if (it != tensor_offsets.end())
...
void parameter_offloader::seat_dense_tensors(offloader_schedule & schedule)
{
    ...
    if (tensor_count > 0)
    {
        ...
        for (size_t i = 0; i < tensor_count; ++i)
        {
            ...
            if (schedule.end_offset[i] != off + slot_bytes)
                throw std::runtime_error("parameter_offloader: solved streamed tensor size is invalid");

            if (schedule.end_offset[i] > arena_stream_size)
                throw std::runtime_error("parameter_offloader: solved streamed tensor exceeds arena stream size: " + std::to_string(schedule.end_offset[i]) + " > " + std::to_string(arena_stream_size));
...
bool llama_offloader_graph_cb(ggml_backend_sched_t sched, struct ggml_cgraph * graph, void * ud)
{
    ...
    if (po->schedule_current.gpu_tensors_in_order.empty() && po->static_dense_order_current.size() == po->gpu2cpu.size())
        return true;
    ...
    //clear the old graph
    po->graph_analysis_next.clear();

    const uint64_t graph_hash = po->hash_dense_graph(sched, graph, po->graph_analysis_next.graph_nodes);
    po->graph_analysis_next.hash = graph_hash;
    ...
    {
        po->analyze_dense_graph(po->graph_analysis_next);
        ...
        if (po->graph_analysis_next.dense_fits_arena)
        {
        ...
        if (po->graph_analysis_next.dense_fits_arena)
        {
            po->schedule_next.clear();
        }
        else
        {
            po->build_next_schedule(po->schedule_next, po->graph_analysis_next);
            po->build_graph_runtime_metadata(po->graph_analysis_next, po->schedule_next, cache_entry);
        ...
        cache_entry.streaming_fit = streaming_fit;
        po->dense_graph_cache.emplace(graph_hash, std::move(cache_entry));
    }
    else  // The graph was found in the cache
    {
        po->static_dense_order = cache_it->second.static_dense_order;
        po->schedule_next = cache_it->second.schedule;
        streaming_fit = cache_it->second.streaming_fit;

        const std::vector<ggml_tensor *> & graph_nodes = po->graph_analysis_next.graph_nodes;

        po->graph_analysis_next.release_node_by_tensor.resize(cache_it->second.release_node_ordinal_by_tensor.size());

        for (size_t i = 0; i < cache_it->second.release_node_ordinal_by_tensor.size(); ++i)
        {
            const int node_ordinal = cache_it->second.release_node_ordinal_by_tensor[i];

            po->graph_analysis_next.release_node_by_tensor[i] = node_ordinal >= 0 ? graph_nodes[(size_t)node_ordinal] : nullptr;
        }

        po->graph_analysis_next.next_required_tensor_idx.clear();
        po->graph_analysis_next.next_required_tensor_idx.reserve(cache_it->second.next_required_tensor_idx_by_node_ordinal.size());

        for (size_t node_ordinal = 0;
            node_ordinal < cache_it->second.next_required_tensor_idx_by_node_ordinal.size();
            ++node_ordinal)
        {
            const int next_required_idx = cache_it->second.next_required_tensor_idx_by_node_ordinal[node_ordinal];

            if (next_required_idx == -2)
                continue;

            po->graph_analysis_next.next_required_tensor_idx.emplace(graph_nodes[node_ordinal], next_required_idx);
        }
    }
